...
#ifdef __FAKE__
#include "bootdata_AR0521_AP1302.h"

struct otp_flash *tevi_ar0521_otp_flash_init(struct i2c_client *client)
{
	return NULL;
}

u16 tevi_ar0521_otp_flash_get_checksum(struct otp_flash *instance)
{
	return BOOTDATA_CHECKSUM;
}

size_t tevi_ar0521_otp_flash_read(struct otp_flash *instance, u8 *data, int addr, size_t len)
{
	size_t l;
...
size_t tevi_ar0521_otp_flash_get_pll_length(struct otp_flash *instance)
{
	return BOOTDATA_PLL_INIT_SIZE;
}

size_t tevi_ar0521_otp_flash_get_pll_section(struct otp_flash *instance, u8 *data)
{
	tevi_ar0521_otp_flash_read(instance, data, 0, BOOTDATA_PLL_INIT_SIZE);
	return BOOTDATA_PLL_INIT_SIZE;
}
#else
/*
...
static int __otp_read(struct i2c_client *client, u32 reg, u16 size, u8 *val, u32 eeprom_addr)
{
	...
	msg[0].addr = eeprom_addr + (reg >> 16);
	msg[0].flags = 0;
	msg[0].buf = buf;
	msg[0].len = sizeof(buf);

	msg[1].addr = eeprom_addr + (reg >> 16);
...
struct otp_flash *tevi_ar0521_otp_flash_init(struct i2c_client *client, u32 eeprom_addr)
{
	...
	ret = __otp_read(client, 0, 1, &__header_ver, eeprom_addr);
	if (ret < 0)
		return ERR_PTR(ret);
...
size_t tevi_ar0521_otp_flash_read(struct otp_flash *instance, u8 *data, int addr, size_t len, u32 eeprom_addr)
{
	u8 *temp;
	struct header_ver2 *header;
	size_t l;
	...
	if(temp[0] == 2) {
		...
		__otp_read(instance->client, addr + header->content_offset, l, data, eeprom_addr);
		return l;
	}
...
size_t tevi_ar0521_otp_flash_get_pll_section(struct otp_flash *instance, u8 *data, u32 eeprom_addr)
{
	u8 *temp;
	struct header_ver2 *header;
	...
	if(temp[0] == 2) {
		...
		if (header->pll_bootdata_len != 0) {
			tevi_ar0521_otp_flash_read(instance, data, 0,
				       header->pll_bootdata_len, eeprom_addr);
		}
		return header->pll_bootdata_len;
	}
