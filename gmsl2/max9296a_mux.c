...
static int max_ser_change_address(struct i2c_client *client, struct regmap *regmap, u8 addr)
{
	...
	if (addr == 0x42) {
		regmap_update_bits(regmap, 0x7b, GENMASK(2, 0), 0x03);
		regmap_update_bits(regmap, 0x83, GENMASK(2, 0), 0x03);
		regmap_update_bits(regmap, 0x8b, GENMASK(2, 0), 0x03);
		regmap_update_bits(regmap, 0x93, GENMASK(2, 0), 0x03);
		regmap_update_bits(regmap, 0xa3, GENMASK(2, 0), 0x03);
		regmap_update_bits(regmap, 0xab, GENMASK(2, 0), 0x03);
	}
