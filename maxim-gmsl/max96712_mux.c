...
static unsigned int max96712_field_get(unsigned int val, unsigned int mask)
{
	return (val & mask) >> __ffs(mask);
}

static unsigned int max96712_field_prep(unsigned int val, unsigned int mask)
{
					...
					unsigned long *config)
{
	...
	case PIN_CONFIG_BIAS_PULL_UP:
		...
		val = max96712_field_get(ret, mask) == val;
		if (!val)
			return -EINVAL;
	...
	case MAX96712_PINCTRL_GMSL_RX_ID_D:
		...
		val = max96712_field_get(val, mask);
		break;
	default:
		return -ENOTSUPP;
	}
						...
						unsigned long config)
{
	...
	case PIN_CONFIG_BIAS_PULL_DOWN:
	case PIN_CONFIG_BIAS_PULL_UP:
		val = max96712_field_prep(val, mask);
	...
	case PIN_CONFIG_INPUT_ENABLE:
	case PIN_CONFIG_OUTPUT:
		val = max96712_field_prep(arg ? val : ~val, mask);
	...
	case MAX96712_PINCTRL_GMSL_RX_ID_C:
	case MAX96712_PINCTRL_GMSL_RX_ID_D:
		val = max96712_field_prep(arg, mask);
...
static int max96712_reset(struct max96712_priv *priv)
{
	...
	if (priv->i2c_addr != priv->client->addr) {
		...
		ret = max_des_wait_for_multiple(client, regmap, max96712_addr, ARRAY_SIZE(max96712_addr));
		if (ret) {
			dev_err(priv->dev,
...
err_unregister_client:
		...
		ret = max96712_update_bits(priv, 0x13, BIT(6), BIT(6));
		if (ret)
			return ret;
...
static int max96712_init_lane_config(struct max96712_priv *priv)
{
	...
	ret = max96712_update_bits(priv, 0x8a0, 0x1f,
				max96712_lane_configs[i].bit);

	/* Force DPHY0 enable as MIPI clock */
	ret += max96712_update_bits(priv, 0x8a0, BIT(5), BIT(5));
	if (ret)
		return ret;
...
static int max96712_check_gmsl_links(struct max_des_priv *des_priv)
{
	...
	max96712_update_bits(priv, 0x6, GENMASK(3, 0), des_priv->gmsl_link_mask);
	...
	while (!time_after(jiffies, timeout)) {
		...
		if ((max96712_read(priv, link_lock_addr[current_link]) & BIT(3)) == BIT(3)) {
...
static int max96712_init(struct max_des_priv *des_priv)
{
	...
	ret = max96712_update_bits(priv, 0xf4, BIT(4),
	...
	ret = max96712_update_bits(priv, 0x641, GENMASK(6, 4), 0x50);
	ret = max96712_update_bits(priv, 0x651, GENMASK(6, 4), 0x50);
	ret = max96712_update_bits(priv, 0x661, GENMASK(6, 4), 0x50);
	ret = max96712_update_bits(priv, 0x671, GENMASK(6, 4), 0x50);
				...
				struct max_des_phy *phy)
{
	...
	if (dpll_freq > 1500000000ull) {
		...
		ret = max96712_write(priv, 0x903 + 0x40 * index, 0x81);
		if (ret)
			return ret;

		/* Enable periodic deskew with 2 x 1k UI.. */
		ret = max96712_write(priv, 0x904 + 0x40 * index, 0x81);
		if (ret)
			return ret;
	} else {
	...
	ret = max96712_update_bits(priv, reg, BIT(0), 0x01);
	if (ret)
		return ret;
	...
	ret = max96712_update_bits(priv, reg, GENMASK(2, 0), val);
	if (ret)
		return ret;
	...
	ret = max96712_update_bits(priv, 0x8a2, mask, mask);
	if (ret)
		return ret;
				...
				struct max_des_pipe *pipe)
{
	...
	mask = BIT(index) << 4;
	ret = max96712_update_bits(priv, 0x414, mask, pipe->dbl8 ? mask : 0);
	if (ret)
		return ret;

	mask = BIT(index) << 4;
	ret = max96712_update_bits(priv, 0x417, mask, pipe->dbl8mode ? mask : 0);
	if (ret)
		return ret;
	...
	mask = BIT(index);
	ret = max96712_update_bits(priv, 0x41f, mask, pipe->dbl12 ? mask : 0);
	if (ret)
		return ret;
				  ...
				  struct max_des_link *link)
{
	...
	ret = max96712_update_bits(priv, 0x1445 + 0x100 * index, GENMASK(2, 0), 0x0);
	if (ret)
		return ret;

	ret = max96712_write(priv, 0x14D1 + 0x100 * index, 0x03);
	if (ret)
		return ret;

	ret = max96712_write(priv, 0x6c2, 0x10);
	if (ret)
		return ret;

	ret = max96712_update_bits(priv, 0x18, BIT(index), BIT(index));
	if (ret)
		return ret;
	msleep(60);
				  ...
				  struct max_des_fsync *fsync)
{
	...
	if (fsync->internal || fsync->internal_output) {
		ret = max96712_write(priv, 0x4b1, 0x00);
		ret += max96712_write(priv, 0x4a2, 0x81);
		ret += max96712_write(priv, 0x4a7, (fsync->freq >> 16) & 0xff);
		ret += max96712_write(priv, 0x4a6, (fsync->freq >> 8) & 0xff);
		ret += max96712_write(priv, 0x4a5, (fsync->freq >> 0) & 0xff);
		ret += max96712_write(priv, 0x4af, 0xcf);
		ret += max96712_write(priv, 0x4a0, fsync->internal_output << 2);
	}
	else if (fsync->external) {
		ret = max96712_write(priv, 0x4a0, fsync->external << 3);
	}
				...
				unsigned int mask)
{
	...
	ret = max96712_update_bits(priv, 0x6, GENMASK(3, 0), mask);
