...
static unsigned int max96716a_field_get(unsigned int val, unsigned int mask)
{
	return (val & mask) >> __ffs(mask);
}

static unsigned int max96716a_field_prep(unsigned int val, unsigned int mask)
{
...
static const struct pinconf_generic_params max96716a_cfg_params[] = {
	{ "maxim,pull-strength-weak", max96716a_PINCTRL_PULL_STRENGTH_WEAK, 0 },
	{ "maxim,jitter-compensation", max96716a_PINCTRL_JITTER_COMPENSATION_EN, 0 },
	{ "maxim,gmsl-tx-a", max96716a_PINCTRL_GMSL_TX_EN_A, 0 },
	{ "maxim,gmsl-rx-a", max96716a_PINCTRL_GMSL_RX_EN_A, 0 },
	{ "maxim,gmsl-tx-id-a", max96716a_PINCTRL_GMSL_TX_ID_A, 0 },
	{ "maxim,gmsl-rx-id-a", max96716a_PINCTRL_GMSL_RX_ID_A, 0 },
	{ "maxim,gmsl-tx-b", max96716a_PINCTRL_GMSL_TX_EN_B, 0 },
	{ "maxim,gmsl-rx-b", max96716a_PINCTRL_GMSL_RX_EN_B, 0 },
	{ "maxim,gmsl-tx-id-b", max96716a_PINCTRL_GMSL_TX_ID_B, 0 },
	{ "maxim,gmsl-rx-id-b", max96716a_PINCTRL_GMSL_RX_ID_B, 0 },
};
					...
					unsigned long *config)
{
	...
	case PIN_CONFIG_BIAS_PULL_UP:
		...
		val = max96716a_field_get(ret, mask) == val;
		if (!val)
			return -EINVAL;
	...
	case max96716a_PINCTRL_GMSL_RX_ID_B:
		...
		val = max96716a_field_get(val, mask);
		break;
	default:
		return -ENOTSUPP;
	}
						...
						unsigned long config)
{
	...
	case PIN_CONFIG_BIAS_PULL_DOWN:
	case PIN_CONFIG_BIAS_PULL_UP:
		val = max96716a_field_prep(val, mask);
	...
	case PIN_CONFIG_INPUT_ENABLE:
	case PIN_CONFIG_OUTPUT:
		val = max96716a_field_prep(arg ? val : ~val, mask);
	...
	case max96716a_PINCTRL_GMSL_TX_ID_B:
	case max96716a_PINCTRL_GMSL_RX_ID_B:
		val = max96716a_field_prep(arg, mask);
...
{
	unsigned long config =
		pinconf_to_config_packed(max96716a_PINCTRL_INPUT_VALUE, 0);
	struct max96716a_priv *priv = gpiochip_get_data(gc);
	int ret;
...
static int max96716a_reset(struct max96716a_priv *priv)
{
	...
	if (priv->i2c_addr != priv->client->addr) {
		...
		ret = max_des_wait_for_multiple(client, regmap, max96716a_addr, ARRAY_SIZE(max96716a_addr));
		if (ret) {
			dev_err(priv->dev,
...
static int max96716a_mipi_enable(struct max_des_priv *des_priv, bool enable)
{
	...
	if (enable) {
		ret = max96716a_update_bits(priv, 0x313, 0x02, 0x02);
		if (ret)
			return ret;
	} else {
		ret = max96716a_update_bits(priv, 0x313, 0x02, 0x00);
		if (ret)
			return ret;
	}
...
static int max96716a_init(struct max_des_priv *des_priv)
{
	...
	/* Disable all pipes. */
	ret = max96716a_update_bits(priv, 0x2, GENMASK(6, 5), 0x00);
	if (ret)
		return ret;

	ret = max96716a_update_bits(priv, 0x160, GENMASK(1, 0), 0x00);
	if (ret)
		return ret;

	/* Set I2C speed to 397Kbps */
	ret = max96716a_update_bits(priv, 0x41, GENMASK(6, 4), 0x50);
	if (ret)
		return ret;

	/* Phy configure to 2x4 */
	ret = max96716a_write(priv, 0x330, 0x04);
	if (ret)
		return ret;

	/* Disable all UART channels */
	ret = max96716a_update_bits(priv, 0x3, GENMASK(5, 4), 0x00);
	if (ret)
		return ret;
				 ...
				 struct max_des_phy *phy)
{
	...
	ret = max96716a_update_bits(priv, 0x44a + 0x40 * phy->index, GENMASK(7, 6),
	...
	ret = max96716a_update_bits(priv, reg, 0xff, val);
	if (ret)
		return ret;
	...
	ret = max96716a_update_bits(priv, reg, 0x3f, val);
	if (ret)
		return ret;

	ret = max96716a_update_bits(priv, 0x325, 0x80, 0x80);
	if (ret)
		return ret;

	/* Put DPLL block into reset. */
	ret = max96716a_update_bits(priv, 0x1c00 + 0x100 * master_phy, BIT(0), 0x00);
	if (ret)
		return ret;

	/* Set DPLL frequency. */
	reg = 0x31d + 0x3 * master_phy;
	ret = max96716a_update_bits(priv, reg, GENMASK(4, 0),
				   div_u64(dpll_freq, 100000000));
	if (ret)
		return ret;

	/* Enable DPLL frequency. */
	ret = max96716a_update_bits(priv, reg, BIT(5), BIT(5));
	if (ret)
		return ret;

	// ret = max96716a_update_bits(priv, reg, GENMASK(7, 6), 0xc0);
	// if (ret)
	// 	return ret;

	/* Pull DPLL block out of reset. */
	ret = max96716a_update_bits(priv, 0x1c00 + 0x100 * master_phy, BIT(0), 0x01);
	if (ret)
		return ret;
	...
	if (dpll_freq > 1500000000ull) {
		...
		ret = max96716a_write(priv, 0x443 + 0x40 * phy->index, 0x81);
		if (ret)
			return ret;

		/* Enable periodic deskew with 2 x 1k UI.. */
		ret = max96716a_write(priv, 0x444 + 0x40 * phy->index, 0x81);
		if (ret)
			return ret;
	} else {
		/* Disable initial deskew. */
		ret = max96716a_write(priv, 0x443 + 0x40 * phy->index, 0x07);
		if (ret)
			return ret;

		/* Disable periodic deskew. */
		ret = max96716a_write(priv, 0x444 + 0x40 * phy->index, 0x01);
		if (ret)
			return ret;
	}
	...
	ret = max96716a_update_bits(priv, 0x433 + 0x40 * master_phy, GENMASK(2, 0), val);

	/* Enable PHY. */
	mask = (BIT(master_phy) | BIT(slave_phy)) << 4;
	ret = max96716a_update_bits(priv, 0x332, mask, mask);
	if (ret)
		return ret;
				  ...
				  struct max_des_pipe *pipe)
{
	...
	/* Enable pipe. */
	mask = BIT(index + 4);
	ret = max96716a_update_bits(priv, 0x2, mask, mask);
	if (ret)
		return ret;

	mask = BIT(index - 1);
	ret = max96716a_update_bits(priv, 0x160, mask, mask);
	if (ret)
		return ret;

	/* Set source stream. */
	mask = GENMASK(2 + 3 * (index - 1), 0 + 3 * (index - 1));
	ret = max96716a_update_bits(
							priv, 0x161, mask,
				  ...
				  struct max_des_link *link)
{
	...
	if (priv->info->supports_tunnel_mode) {
		mask = BIT(0);
		ret = max96716a_update_bits(priv, 0x474 + 0x40 * index, mask,
				  ...
				  struct max_des_fsync *fsync)
{
	...
	if (fsync->internal || fsync->internal_output) {
		ret = max96716a_write(priv, 0x3f1, 0x00);
		ret += max96716a_write(priv, 0x3e2, 0x01);
		ret += max96716a_write(priv, 0x3ea, 0x00);
		ret += max96716a_write(priv, 0x3eb, 0x00);
		ret += max96716a_write(priv, 0x3e7, (fsync->freq >> 16) & 0xff);
		ret += max96716a_write(priv, 0x3e6, (fsync->freq >> 8) & 0xff);
		ret += max96716a_write(priv, 0x3e5, (fsync->freq >> 0) & 0xff);
		ret += max96716a_write(priv, 0x3ef, 0xc0);
		ret += max96716a_write(priv, 0x3e0, fsync->internal_output << 2);
	}
	else if (fsync->external) {
		ret = max96716a_write(priv, 0x3e0, fsync->external << 3);
	}
					...
					unsigned int i)
{
	...
	val = BIT(i % 8);
	ret = max96716a_update_bits(priv, reg, val, val);
	if (ret)
		return ret;
...
static int max96716a_post_init(struct max_des_priv *des_priv)
{
	...
	for_each_subdev(des_priv, sd_priv) {
		...
		dev_dbg(des_priv->dev, "%s(): %s, pipe_id [%d], phy_id [%d], src_vc_id [%d], dst_vc_id [%d] fmt [0x%04x]\n",
			...
			sd_priv->src_vc_id, sd_priv->dst_vc_id, fmt->code);
...
static int max96716a_remove(struct i2c_client *client)
{
...
static struct i2c_driver max96716a_i2c_driver = {
	...
	.probe_new = max96716a_probe,
	.remove = max96716a_remove,
};

module_i2c_driver(max96716a_i2c_driver);

MODULE_DESCRIPTION("Maxim MAX96716A Dual GMSL2 Deserializer Driver");
